...
import json
import os
from models import CrawlRequest, SearchRequest, Website
...
@app.post("/crawl")
def crawl_endpoint(request: CrawlRequest, service: FirecrawlService = Depends(get_firecrawl_service)):
    """
    Crawl a website.
    Uses Firecrawl API.
...
@app.post("/search")
def search_endpoint(request: SearchRequest, service: FirecrawlService = Depends(get_firecrawl_service)):
    """
    Search for a query.
    Uses Firecrawl API.
    Checks cache first to save quota.
    ...
    try:
        result = service.search(
            query=request.query,
            search_options=request.searchOptions,
            page_options=request.pageOptions,
            force_refresh=request.force_refresh
        )
        return result
